(() => {
    ...
    const HM = { h: [40, 60], hl: [212, 55], hr: [330, 55] };
    function hmStyle(pfx, lvl) {
        const [h, s] = HM[pfx]; const t = lvl / 10;
        return `background:hsl(${h} ${s * t}% ${6 + t * 10}%);border:1px solid hsl(${h} ${s}% ${10 + t * 28}%/${.3 + t * .7});color:hsl(${h} ${s * (.3 + t * .7)}% ${15 + t * 70}%)`;
    }
    ...
    function renderKB(el, mode, ov) {
        ...
        const lbl = mode === 'split' ? '<div class="split-label"><span>← 子音 (左手)</span><span>母音 (右手) →</span></div>' : '';
        el.innerHTML = `<section class="kb${mode === 'split' ? ' split' : ''}">${rows}${lbl}</section>`;
    }
    ...
    const SLIDES = [
        {
            ...
            kb: 'normal',
            interactive: true,
            states: { 0: { 5: 'rn', 6: 'rn', 7: 'rn', 8: 'rn', 9: 'rn', 10: 'rn', 11: 'rn' }, 1: { 5: 'rn', 10: 'rn' }, 2: { 5: 'rn', 7: 'rn', 8: 'rn', 9: 'rn' } }
        },
        {
            h3: 'QWERTY配列とは',
